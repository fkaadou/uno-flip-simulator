...
from classes import Card, Holder, Player_official
import sys
import numpy as np
...
for j in tqdm(range(N)):
    ...
    while not winner:
        ...
        for player in all_players:
            ...
            if action == 'flip':       
                ...
                if mode == 'light':
                    mode = 'dark'
                    while not discard.top_card().val_dark.isnumeric():
                        discard.add(deck.draw(reserve=discard))
                elif mode == 'dark':
                    mode = 'light'
                    while not discard.top_card().val_light.isnumeric():
                        discard.add(deck.draw(reserve=discard))
    ...
    if j==int(0.25*N) or j==int(0.5*N) or j==int(0.75*N) or j==int(0.95*N):
        ...
        original_stdout.write(f'Elapsed time: {end-start} s\n')
